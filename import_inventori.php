...
if (isset($_POST['import_submit'])) {
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
        ...
        $domain_raw             = $sheet->getCell('C'.$row)->getCalculatedValue() ?? ''; //  KOLOM BARU
        $type_raw               = $sheet->getCell('D'.$row)->getCalculatedValue() ?? '';
        $device_category_raw    = $sheet->getCell('E'.$row)->getCalculatedValue() ?? ''; //  KOLOM BARU
        $rak_excel_raw          = $sheet->getCell('F'.$row)->getCalculatedValue() ?? ''; //  BERGESER
        $ram_raw                = $sheet->getCell('G'.$row)->getCalculatedValue() ?? ''; //  BERGESER
        $storage_raw            = $sheet->getCell('H'.$row)->getCalculatedValue() ?? ''; //  BERGESER
        $win_raw                = $sheet->getCell('I'.$row)->getCalculatedValue() ?? ''; //  BERGESER
        $keterangan_raw         = $sheet->getCell('J'.$row)->getCalculatedValue() ?? ''; //  BERGESER
        $kelengkapan_raw        = $sheet->getCell('K'.$row)->getCalculatedValue() ?? ''; //  BERGESER
        ...
        $nik_raw                = $sheet->getCell('N'.$row)->getCalculatedValue() ?? ''; //  BERGESER
        $nama_raw               = $sheet->getCell('O'.$row)->getCalculatedValue() ?? ''; //  BERGESER
        $divisi_raw             = $sheet->getCell('P'.$row)->getCalculatedValue() ?? ''; //  BERGESER
        ...
        // ===========================================
        // 4. PENANGANAN WEBSOCKET DAN REDIRECT AKHIR
        // ===========================================

            //  TAMBAH: Kirim sinyal BULK INSERT ke WebSocket
            // ID diisi 0 karena kita tidak peduli ID mana yang berubah, hanya perlu REFRESH.
            // Pastikan fungsi ini ada di helpers.php
