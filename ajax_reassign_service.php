...
try {
    ...
    if (!$stmt_admin) {
        throw new Exception("Prepare Admin Gagal: " . $koneksi->error);
    }
    ...
    if (!$stmt_update) {
        throw new Exception("Prepare Update Service Gagal: " . $koneksi->error);
    }
    ...
    if (!$stmt_get_inventori_id) {
        throw new Exception("Prepare Get Inventori ID Gagal: " . $koneksi->error);
    }
    ...
    $stmt_log = $koneksi->prepare("INSERT INTO histori_aset (inventori_id, tanggal, aksi, oleh, catatan)
                 VALUES (?, ?, ?, ?, ?)");
    if (!$stmt_log) {
        throw new Exception("Prepare Log Histori Gagal: " . $koneksi->error);
    }
