...
tipeLaptopModal.addEventListener('show.bs.modal', function () {
    ...
    fetch(apiUrl)
        .then(response => {
            ...
            if (!response.ok) {
                throw new Error('Gagal mengambil data tipe. Status: ' + response.status);
            }
        ...
        .then(data => {
            ...
            if (data.length > 0) {
                ...
                htmlContent += '<thead class="table-light"><tr><th>ID</th><th>Nama Tipe</th><th class="text-center">Aksi</th></tr></thead>';
                htmlContent += '<tbody>';
                ...
                htmlContent = '<p class="text-muted text-center mt-4">Belum ada tipe inventori yang ditambahkan.</p>';
            }
