...
document.addEventListener("DOMContentLoaded", function () {
	...
	// --- 1.1 VALIDASI ELEMEN (VERSI AMAN) ---
	if (!activityModalElement) {
		console.warn(
			"JS Aktivitas: Modal utama (#aktivitasModal) tidak ditemukan. Script berhenti.",
		);
		return;
	}

	const safeListener = (el, event, callback) => {
		if (el) el.addEventListener(event, callback);
	};

	if (!toggleLoan)
		console.warn(
			"JS Aktivitas: Elemen #toggleLoan tidak ditemukan (Fitur Loan nonaktif).",
		);
	if (!inputAksiSelect)
		console.warn("JS Aktivitas: Elemen #inputAksiSelect tidak ditemukan.");
	...
	function renderAssetTable() {
		...
		const searchTerm = searchTableInput
			? searchTableInput.value.toLowerCase()
			: "";

		// 1. Filter Data
		filteredData = window.loanAssetsData.filter((asset) => {
			const matchStatus =
				filterStatus === "" || filterStatus === "available"
					? true
	...
	function handleLoanToggle() {
		if (this.checked) {
			...
			document.querySelector(
				'#inputAksiSelect option[value="Diservis"]',
			).style.display = "none";
			...
			aksiHelpText.textContent =
				"Aksi akan default ke 'Mencatat Servis' jika tanpa Loan.";
	...
	} // Live Search di dalam modal (Sudah aman karena sudah pakai IF)
				...
				.then((response) => {
					if (!response.ok) {
						return response.json().then((errorData) => {
							throw new Error(
								errorData.message || "Server Error. Cek kode PHP.",
							);
						});
					}
